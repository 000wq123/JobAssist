...
| Key                      | Value                                                                  |
|--------------------------|------------------------------------------------------------------------|
| `DATABASE_URL`           | `postgresql+asyncpg://neondb_owner:***@ep-noisy-scene-b1tap4ls-pooler.c-5.eu-central-1.aws.neon.tech/neondb?ssl=require` — **not currently in `backend/.env`** (that file now points at local SQLite `./dev.db` for dev; leave it that way). Grab the Frankfurt URL fresh from the **Neon dashboard** (Project → Connect → Pooled connection), rewrite `postgresql://` → `postgresql+asyncpg://`, use **`?ssl=require`** (asyncpg accepts `ssl`, not `sslmode`/`channel_binding` — those crash with `TypeError: connect() got an unexpected keyword argument '…'`; the code strips/normalizes them as a safety net, but prefer `ssl`), and keep the `-pooler` host (pooled = correct for Railway). |

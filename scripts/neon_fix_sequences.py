...
async def main():
    conn = await asyncpg.connect(NEW.replace("postgresql+asyncpg://","postgresql://").split("?")[0], ssl="require")

    # find all tables with an integer 'id' column lacking a default
    ...
    for r in rows:
        ...
        try:
            ...
            await conn.execute(f'SELECT setval('"{seq}"', COALESCE((SELECT MAX(id) FROM "{t}"), 1))')
            await conn.execute(f'ALTER TABLE "{t}" ALTER COLUMN id SET DEFAULT nextval('"{seq}"')')
            print(f"  fixed: {t}")
